...
export const warehousesApi = {
    list: async (): Promise<WarehouseItem[]> => {
        const { data, error } = await api.inventory.warehouses.get();
        if (error) throwApiError(error);
        return data as WarehouseItem[];
    },

    get: async (id: number) => {
        const { data, error } = await api.inventory.warehouses({ id }).get();
        if (error) throwApiError(error);
        return data!;
    },

    create: async (body: CreateWarehouseBody) => {
        const { data, error } = await api.inventory.warehouses.post(body as any);
        if (error) throwApiError(error);
        return data!;
    },

    update: async (id: number, body: UpdateWarehouseBody) => {
        const { data, error } = await (api.inventory.warehouses as any)({ id }).put(body);
        if (error) throwApiError(error);
        return data!;
    },

    deactivate: async (id: number) => {
        const { data, error } = await (api.inventory.warehouses as any)({ id }).deactivate.patch();
        if (error) throwApiError(error);
        return data!;
    },

    restore: async (id: number) => {
        const { data, error } = await (api.inventory.warehouses as any)({ id }).restore.patch();
        if (error) throwApiError(error);
        return data!;
    },
};
