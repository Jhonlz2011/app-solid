...
DECLARE
    ...
    app_company_id VARCHAR;
    action_type VARCHAR;
    record_id_val VARCHAR;
    parsed_user_id UUID := NULL;
    parsed_company_id INT := NULL;
BEGIN
    -- 1. Capturar contexto inyectado desde la transacción de Drizzle/Bun
    app_user_id := NULLIF(current_setting('app.user_id', true), '');
    app_ip_address := NULLIF(current_setting('app.ip_address', true), '');
    app_company_id := NULLIF(current_setting('app.current_company_id', true), '');

    -- 2. Conversión segura y eficiente a UUID (O(1) nativo sin costo de regex)
    IF app_user_id IS NOT NULL THEN
        BEGIN
            parsed_user_id := app_user_id::UUID;
        EXCEPTION WHEN invalid_text_representation THEN
            parsed_user_id := NULL;
        END;
    END IF;

    -- 3. Conversión segura a company_id
    IF app_company_id IS NOT NULL THEN
        BEGIN
            parsed_company_id := app_company_id::INTEGER;
        EXCEPTION WHEN invalid_text_representation THEN
            parsed_company_id := NULL;
        END;
    END IF;

    -- 4. Extracción de valores por operación
    IF (TG_OP = 'DELETE') THEN
        action_type := 'DELETE';
        record_id_val := OLD.id::VARCHAR;
        IF parsed_company_id IS NULL AND (row_to_json(OLD)::jsonb ? 'company_id') THEN
            parsed_company_id := (OLD.company_id)::INTEGER;
        END IF;
    ELSIF (TG_OP = 'UPDATE') THEN
        action_type := 'UPDATE';
        record_id_val := NEW.id::VARCHAR;
        IF (row_to_json(NEW)::jsonb ? 'company_id') THEN
            parsed_company_id := (NEW.company_id)::INTEGER;
        END IF;
    ...
    -- 5. Inserción rápida en la cola temporal sin bloqueo
    INSERT INTO _audit_queue (
        table_name, record_id, action, old_data, new_data, user_id, ip_address, company_id
    ) VALUES (
        TG_TABLE_NAME,
        record_id_val,
        action_type,
    ...
    -- 6. Notificación asíncrona de baja sobrecarga (solo se emite en COMMIT)
